/**
  *********************************************************************************************************************************************************
  @file     :grid.h
  @brief    :Header of the grid class for time plots
  *********************************************************************************************************************************************************
...
{
public:
    Grid(appPreferencesStruct *pref, QString title, int width, int height, fontManager *font);
    ~Grid();
    ...
    void recreate_Grid(float max_Y, float min_Y);
    void recreate_Grid();
    void update_Text(int width, int height);

    float get_ConvFact() { return conversion_factor; }
    float get_StepX() { return step_x; }
    float get_X_Axis() { return x_axis_position; }
    QImage getFontTexture() { return fontTexture; }

    int get_N_points() { return n_points; }
    void set_N_points(int N_points) { if (N_points >= 2) n_points = N_points; }
...
private:
    ...
    float conversion_factor;  //transforms input data into y-positions
    float step_x;  //step_x gives the step between two points along the x axis
