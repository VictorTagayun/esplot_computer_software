/**
  *********************************************************************************************************************************************************
  ...
  @brief    :Functions for Plot Window class
  *********************************************************************************************************************************************************
...
plot_Window::plot_Window(QString title, int index, double frequency, appPreferencesStruct *pref, fontManager *font)
{
    plotIndex = index;
    ...
    glPlot->setGridTimeBase(1.0 / frequency);
    connect(glPlot, &GLWindow::updateSigProperties, this, &plot_Window::updateSigProperties);
    ...
    connect(triggerLvlSB, SIGNAL (editingFinished()), this, SLOT (triggerLvlchanged()));
...
void plot_Window::createSpinBoxes()
{
    QLocale locale;
    double time;
    QString text;
    ...
    triggerLvlSB = new QDoubleSpinBox(this);
    ...
    N_Samples = glPlot->get_Grid()->get_N_points();
    ...
    triggerLvlSB->setMinimum(static_cast<double>(std::numeric_limits<float>::max() * -1));
    triggerLvlSB->setMaximum(static_cast<double>(std::numeric_limits<float>::max()));
    triggerLvlSB->setValue(0.0);
    triggerLvlSB->setSingleStep(1.0);
    triggerLvlSB->resize(triggerLvlSB->sizeHint());
    ...
    QLabel *triggerLabel = new QLabel("Trigger value:");
    ...
    vert6->addWidget(triggerLabel); vert6->addWidget(triggerLvlSB);
    vert7->addWidget(autoscaleLabel); vert7->addWidget(autoscaleSlider);
    vert8->addWidget(pastValueLabel); vert8->addWidget(pastValueSlider);

    QVBoxLayout *vv1 = new QVBoxLayout;
    QVBoxLayout *vv2 = new QVBoxLayout;
    QVBoxLayout *vv3 = new QVBoxLayout;

    vv1->addLayout(vert2); vv1->addLayout(vert1); vv1->addLayout(vert8);
    vv2->addLayout(vert4); vv2->addLayout(vert5); vv2->addLayout(vert7);
    vv3->addLayout(vert3); vv3->addStretch(); vv3->addLayout(vert6);
...
void plot_Window::thread_prepare_signal(int i, QVector<int> points, QVector<float> floats, QVector<void *> pointers)
{
    (void) floats;
    ...
    try   //insert to catch out of buffer accesses and/or divisions by zero
    {
        if (StatsEnabled == true)
        {
            ...
            sig_properties[i].stats.max = buff_ptr[i][start];
            sig_properties[i].stats.min = buff_ptr[i][start];
...
void plot_Window::find_min_max_signals(int n_signals, int start, int end, float **buff_ptr, float *Min, float *Max)
{
    float min, max;
    int i, j;
    bool found;
...
void plot_Window::addSignal(uint32_t index, QString name, QColor color)
{
    SigProperty p;
    ...
    QMenu *menu;
    menu = triggerSourceSignal->menu();
    menu->addAction(p.triggerAct);
    connect(sig_properties[sig_properties.count() - 1].triggerAct, SIGNAL (triggered()), this, SLOT (triggerSourceChanged()));

    if (sig_properties.count() == 1)
    {
        sig_properties[0].triggerAct->setChecked(true);
        triggerSourceIndex = 0;  //it's the first element
    }

    glPlot->setSigProperties(sig_properties);
}

void plot_Window::removeSignal(uint32_t index)
{
    int i; int idx;
    ...
    triggerSourceSignal->menu()->removeAction(sig_properties[idx].triggerAct);
    if (triggerSourceIndex == idx)
        triggerSourceIndex = -1;  //no trigger active
    sig_properties.remove(idx);
    glPlot->setSigProperties(sig_properties);
}

void plot_Window::clearAllSignal()
{
...
void plot_Window::setNPoints(int N)
{
    if (N > 1)
    {
...
void plot_Window::setTitle(QString title)
{
...
void plot_Window::setGrid(bool en)
{
...
int plot_Window::parallel_prepare_Signal_Data(int n_signals, int n_points, float **buff_ptr, QColor *colors, float *line_widths)
{
    int i; int n_p;
    float step_x;
    float x_axis;
    ...
    //in this case, before we proceed we have to look for a trigger event and establish the start and end position of the buffer
    if ((TriggerEnabled == true) && (triggerSourceIndex != -1) && (n_points > glPlot->get_Grid()->get_N_points()))  //trigger is enabled, a source has been selected, there are more points than the grid
    {
        //first we define the portion of the buffer we are going to scan
        middle = start + (n_p >> 1);  //we get the middle index. we will first look for trigger event moving left and if it is not found move right
        trigger_found = false; i = middle;
        while ((trigger_found == false) && (i >= (start + 1)))
        {
            if (trigger_mode == 0)  // we are looking for a rise event
            {
                if (((buff_ptr[triggerSourceIndex][i] - trigger_level) >= 0) && ((buff_ptr[triggerSourceIndex][i - 1] - trigger_level) < 0))  //trigger found
                    trigger_found = true;
            }
            if (trigger_mode == 1)  // we are looking for a fall event
            {
                if (((buff_ptr[triggerSourceIndex][i] - trigger_level) <= 0) && ((buff_ptr[triggerSourceIndex][i - 1] - trigger_level) > 0))  //trigger found
                    trigger_found = true;
            }
        ...
        if (trigger_found == true)
            i = i + 1;
        //if we have not found the trigger, we try to move towards right
        if (trigger_found == false)
        {
            i = middle;
            while ((trigger_found == false) && (i < (n_points - 1)))
            {
                if (trigger_mode == 0)  // we are looking for a rise event
                {
                    if (((buff_ptr[triggerSourceIndex][i] - trigger_level) <= 0) && ((buff_ptr[triggerSourceIndex][i + 1] - trigger_level) > 0))  //trigger found
        ...
        //if we still have not found the trigger, we do not do anything and plot the last samples
        //otherwise we define the indexes around the found index and plot that window
        if (trigger_found == true)
        {
            start = i - ((n_p * trigger_position) / 100);
            end = start + n_p - 1;
            if (start < 0)
                start = 0;
            if (end >= n_points)
    ...
    if (Autoscale == true)  //in this case, we need to rewrite the grid according to the min and max values of the signals to be plotted
    {
        ...
        gridChanged();  //signals a change in the grid
    }
    else
    {
        glPlot->get_Grid()->recreate_Grid();
...
#ifdef USE_VERTEX_ID
    signal_buffer.resize(n_signals * (end - start + 1));  //not times 2, we save only Y values now
#else
    ...
    {
        res[i] = QtConcurrent::run(this, &plot_Window::thread_prepare_signal, i, points, floats, pointers);
    }
...
void plot_Window::keyPressEvent(QKeyEvent *ev)
{
    bool state;
...
void plot_Window::closeWindow()
{
    close();
}

void plot_Window::triggerAutoscale()
{
...
void plot_Window::triggerZoomXY()
{
...
void plot_Window::triggerLegend()
{
...
void plot_Window::triggerRise()
{
    if (triggerRiseAct->isChecked() == true)
    {
        triggerFallAct->setChecked(false);
        TriggerEnabled = true;
        trigger_mode = 0;
        glPlot->enable_trigger(true);
    }
    else
    {
        TriggerEnabled = false;
        trigger_mode = 0;
        glPlot->enable_trigger(false);
    }
}

void plot_Window::triggerFall()
{
    if (triggerFallAct->isChecked() == true)
    {
        triggerRiseAct->setChecked(false);
        TriggerEnabled = true;
        trigger_mode = 1;
        glPlot->enable_trigger(true);
    }
...
void plot_Window::triggerResetStats()
{
    int i;
...
void plot_Window::triggerPosRight()
{
    trigger_position += 5;
    if (trigger_position > 100)
        trigger_position = 100;
    glPlot->set_trigger_pos(trigger_position);
    emit replot(plotIndex);
}

void plot_Window::triggerPosLeft()
{
    trigger_position -= 5;
...
void plot_Window::triggerPosCenter()
{
    trigger_position = 50;
    glPlot->set_trigger_pos(trigger_position);
    emit replot(plotIndex);
...
void plot_Window::triggerYTicks()
{
...
void plot_Window::triggerXTicks()
{
...
void plot_Window::triggerSourceChanged()
{
    //look for which signals has been triggered
    int i, idx;
    idx = -1;

    for (i = 0; i < sig_properties.count(); i++)
        if (sig_properties[i].triggerAct->isChecked() == true)
            idx = i;

    if (idx != -1)
        triggerSourceIndex = idx;
}

void plot_Window::minYSBchanged()
{
...
void plot_Window::maxYSBchanged()
{
...
void plot_Window::nsamplesSBchanged()
{
    double time;
    QLocale locale;
    ...
    if (nSamplesSB->isEnabled() == true)
    {
        ...
        time = static_cast<double>(glPlot->get_Grid()->get_N_points()) / windowFrequency;
        statLabel->setText("Window time interval is " + locale.toString(time, 'g', 3) + " sec");
        emit replot(plotIndex);
        emit gridNSamplesChanged(nSamplesSB->value());
    }
}

void plot_Window::nsamplesSliderchanged(int value)
{
    (void) value;
...
void plot_Window::nsamplesSliderreleased()
{
...
void plot_Window::triggerLvlchanged()
{
    glPlot->set_trigger_level(static_cast<float>(triggerLvlSB->value()));
    trigger_level = static_cast<float>(triggerLvlSB->value());
    emit replot(plotIndex);
}

void plot_Window::xGridRatioChanged(int value)
{
    double v;
...
void plot_Window::yGridRatioChanged(int value)
{
    double v;
    v = 5.0 * pow(2, static_cast<double>(value));
...
void plot_Window::autoscaleCenterChanged(int value)
{
    autoscaleType = value;
    emit replot(plotIndex);
}

void plot_Window::pastValueGainChanged(int value)
{
    float v;
...
void plot_Window::updateSigProperties(QVector<SigProperty> properties)
{
    sig_properties = properties;
}

void plot_Window::gridChanged()
{
...
void plot_Window::createActions()
{
    QSize iconSize(32, 32);

    triggerSignals.clear();

    QMenu *trgSourceMenu = new QMenu(this);
    triggerSourceSignal = new QAction(this);
    triggerSourceSignal->setText("Trigger &Active Source...");
    triggerSourceSignal->setMenu(trgSourceMenu);
    ...
    connect(exportFigAct, &QAction::triggered, this, &plot_Window::exportFigure);
    ...
    connect(closeAct, &QAction::triggered, this, &plot_Window::closeWindow);
    ...
    connect(gridAct, &QAction::triggered, this, &plot_Window::triggerGrid);
    ...
    connect(yTicksAct, &QAction::triggered, this, &plot_Window::triggerYTicks);
    ...
    connect(xTicksAct, &QAction::triggered, this, &plot_Window::triggerXTicks);
    ...
    connect(titleAct, &QAction::triggered, this, &plot_Window::triggerTitle);
    ...
    connect(autoscaleAct, &QAction::triggered, this, &plot_Window::triggerAutoscale);

    triggerRiseAct = new QAction(this);
    triggerRiseAct->setIcon(QIcon(":/Icons/Icons/triggerRise.ico"));
    triggerRiseAct->setCheckable(true);
    triggerRiseAct->setChecked(false);
    triggerRiseAct->setToolTip("Trigger on rising edge");
    triggerRiseAct->setText("&Trigger on rising");
    connect(triggerRiseAct, &QAction::triggered, this, &plot_Window::triggerRise);

    triggerFallAct = new QAction(this);
    triggerFallAct->setIcon(QIcon(":/Icons/Icons/triggerFall.ico"));
    triggerFallAct->setCheckable(true);
    triggerFallAct->setChecked(false);
    triggerFallAct->setToolTip("Trigger on falling edge");
    triggerFallAct->setText("&Trigger on falling");
    connect(triggerFallAct, &QAction::triggered, this, &plot_Window::triggerFall);

    triggerPosRightAct = new QAction(this);
    triggerPosRightAct->setIcon(QIcon(":/Icons/Icons/triggerRight.ico"));
    triggerPosRightAct->setCheckable(false);
    triggerPosRightAct->setToolTip("Move trigger position to the right");
    triggerPosRightAct->setText("Move right");
    connect(triggerPosRightAct, &QAction::triggered, this, &plot_Window::triggerPosRight);

    triggerPosLeftAct = new QAction(this);
    triggerPosLeftAct->setIcon(QIcon(":/Icons/Icons/triggerLeft.ico"));
    triggerPosLeftAct->setCheckable(false);
    triggerPosLeftAct->setToolTip("Move trigger position to the left");
    triggerPosLeftAct->setText("Move left");
    connect(triggerPosLeftAct, &QAction::triggered, this, &plot_Window::triggerPosLeft);

    triggerPosCenterAct = new QAction(this);
    triggerPosCenterAct->setIcon(QIcon(":/Icons/Icons/triggerReset.ico"));
    triggerPosCenterAct->setCheckable(false);
    triggerPosCenterAct->setToolTip("Reset trigger position to the center");
    triggerPosCenterAct->setText("Reset to center");
    connect(triggerPosCenterAct, &QAction::triggered, this, &plot_Window::triggerPosCenter);
    ...
    connect(zoomXYAct, &QAction::triggered, this, &plot_Window::triggerZoomXY);
    ...
    connect(zoomXAct, &QAction::triggered, this, &plot_Window::triggerZoomX);
    ...
    connect(zoomYAct, &QAction::triggered, this, &plot_Window::triggerZoomY);
    ...
    panAct->setText("&Pan");
    connect(panAct, &QAction::triggered, this, &plot_Window::triggerPan);
    ...
    connect(legendAct, &QAction::triggered, this, &plot_Window::triggerLegend);

    statAct = new QAction(this);
    statAct->setIcon(QIcon(":/Icons/Icons/Stats.ico"));
    statAct->setCheckable(true);
    statAct->setChecked(false);  //checked by default
    statAct->setToolTip("Show statistics");
    statAct->setText("&Statistics");
    connect(statAct, &QAction::triggered, this, &plot_Window::triggerStats);
    ...
    connect(resetStatAct, &QAction::triggered, this, &plot_Window::triggerResetStats);
    ...
    toolBar->addSeparator();
    toolBar->addAction(triggerRiseAct);
    toolBar->addAction(triggerFallAct);
    toolBar->addAction(triggerPosLeftAct);
    toolBar->addAction(triggerPosRightAct);
    toolBar->addAction(triggerPosCenterAct);
    toolBar->addSeparator();
    ...
    toolMenu->addAction(triggerSourceSignal);
    toolMenu->addAction(triggerRiseAct);
    toolMenu->addAction(triggerFallAct);
    toolMenu->addAction(triggerPosLeftAct);
    toolMenu->addAction(triggerPosRightAct);
    toolMenu->addAction(triggerPosCenterAct);
